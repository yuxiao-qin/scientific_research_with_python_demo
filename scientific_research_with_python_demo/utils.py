...
def input_parameters(par2ph, step, Num_search, param_orig, para_name):
    # for i, key in enumerate(para_name):
    # data_set = {key:{"par2ph":par2ph[i],"Num_search": Num_search[i], "step":step[i],"param_orig": param_orig[i]}}
    data_set = {key: (par2ph[i], Num_search[i], step[i], param_orig[i]) for i, key in enumerate(para_name)}
